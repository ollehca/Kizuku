...
class IntegrationTester {
  ...
  async testMenuActionIntegration() {
    ...
    const hasMenuSystem = output.includes('PenPot Desktop starting') && 
                         !output.includes('Menu creation failed');
    ...
    this.results.push({
      ...
      details: hasMenuSystem ? 'Menu system initialized successfully' : 'Menu system failed to initialize'
    });
  }
  ...
  async testKeyboardShortcutRegistration() {
    ...
    try {
      ...
      const figmaShortcuts = manager.getShortcutsList().filter(s => 
        ['v', 'r', 'o', 't', 'l', 'p', 'f'].includes(s.shortcut)
      ).length;
      
      const platformShortcuts = manager.getShortcutsList().filter(s =>
        s.shortcut.includes(process.platform === 'darwin' ? 'cmd' : 'ctrl')
      ).length;
  ...
  async testCrossPlatformCompatibility() {
    ...
    try {
      ...
      const hasCorrectModifier = shortcutsContent.includes(`modifierKey = '${expectedModifier}'`) || 
  ...
  async testFileOperationHandling() {
    ...
    try {
      ...
      const fileOperationCount = [hasOpenDialog, hasSaveDialog, hasFileFilters, hasPenpotFilter, hasImageFilter]
  ...
  async testElectronSecurityBestPractices() {
    ...
    try {
      ...
      const hasNoRemoteModule = mainContent.includes('enableRemoteModule: false') || 
                               !mainContent.includes('enableRemoteModule: true');
      
      const securityScore = [hasContextIsolation, hasNodeIntegrationDisabled, hasPreload, hasContextBridge, hasNoRemoteModule]
        .filter(Boolean).length;
  ...
  async testPenPotIntegrationReadiness() {
    ...
    try {
      ...
      const hasHandlerRegistration = preloadContent.includes('registerDesktopMenuHandler') ||
      ...
      const integrationScore = [hasCustomEvents, hasDesktopGlobals, hasHandlerRegistration, hasElectronAPI, hasPlatformInfo]
        .filter(Boolean).length;
