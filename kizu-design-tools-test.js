...
class KizuDesignToolsTester {
  ...
  _setupLaunchValidation(electron, launchState, resolve) {
    setTimeout(() => {
      ...
      if (validation.success) {
        resolve({
          success: true,
          details: `Kizu design interface ready (PID: ${electron.pid})`,
          ...validation.result,
        });
      } else {
        resolve({
  ...
  async testDrawingToolKeyboardShortcuts() {
    if (!this._isElectronProcessRunning()) {
      return { success: false, error: 'Kizu not running - cannot test drawing tool shortcuts' };
    }
  ...
  async testZoomControlsResponsiveness() {
    if (!this._isElectronProcessRunning()) {
      return { success: false, error: 'Kizu not running - cannot test zoom controls' };
    }
  ...
  async testPerformanceVsWeb() {
    if (!this._isElectronProcessRunning()) {
      return { success: false, error: 'Kizu not running - cannot test performance' };
    }
