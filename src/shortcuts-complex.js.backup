...
class ShortcutManager {
  ...
  setupDefaultShortcuts() {
    ...
    this.register(this.platform === 'darwin' ? `${this.modifierKey}+shift+z` : `ctrl+y`, 'redo', 'global', 'Redo');
    ...
    // SPECIAL MODIFIERS (handled differently)
  ...
  buildShortcutString(event) {
    ...
    if (event.ctrlKey && this.platform !== 'darwin') parts.push('ctrl');
    if (event.metaKey && this.platform === 'darwin') parts.push('cmd');
    ...
    if (key !== 'control' && key !== 'meta' && key !== 'alt' && key !== 'shift') {
      ...
      const keyMap = {
        ...
        '+': 'plus',
        '-': 'minus',
        '=': 'plus', // + key without shift on many keyboards
        "'": "'"
      };
  ...
  executeAction(action, event) {
    // Send to both menu system and custom event system
    if (window.electronAPI) {
      window.electronAPI.onMenuAction?.(action);
    }
    
    // Dispatch custom event for PenPot integration
    ...
    // Try direct handler if registered
