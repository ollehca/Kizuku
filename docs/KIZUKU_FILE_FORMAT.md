...
The `.kizuku` file format is a JSON-based container for Kizuku design projects. It provides a portable, self-contained project format that includes design data, assets, metadata, and version information.
...
```
project.kizuku (uncompressed JSON)
project.kizuku.gz (gzip compressed)
```
...
Project structure:
```
project.kizuku          # Main project file
assets/
...
```javascript
async function exportToPenpot(kizukuProject) {
  return {
    name: kizukuProject.metadata.name,
    pages: convertKizuPages(kizukuProject.data.pages),
    // ... PenPot format
  };
}
```
...
await fs.writeFile('project.kizuku', json);
```

### Load Project

```javascript
const fileContent = await fs.readFile('project.kizuku', 'utf-8');
const project = JSON.parse(fileContent);
...
// Validate structure
if (project.type !== 'kizuku-project') {
...
function validateProject(project) {
  ...
  if (project.type !== 'kizuku-project') {
    throw new Error('Not a Kizuku project');
  }
